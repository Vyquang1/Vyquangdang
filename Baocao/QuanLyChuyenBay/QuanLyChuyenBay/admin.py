...
class FlightView(AuthenticatedModelView):
    ...
    # form_columns = ('ten_chuyen_bay', 'san_bay_den')
    # form_args = dict(
    #     san_bay_den=dict(query_factory=lambda: SanBay.query.order_by(SanBay.ten_san_bay))
    # )
...
class RedirectBanVeView(BaseView):
    @expose('/')
    def index(self):
        return redirect('/')
...
admin.add_view(RedirectBanVeView(name='Trang bán vé', endpoint='banve'))
